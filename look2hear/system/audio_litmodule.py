...
class AudioLightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        mixtures, targets, reverb_targets, rir_target, _ = batch
        ...
        out = self(mixtures)
        est_sources = out['x_sep']
        est_derev = out['x_derev']
        est_rir = out['rir']
        loss = self.loss_func["train"](est_sources, est_derev, est_rir, targets, reverb_targets, rir_target)
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx):
        ...
        if dataloader_idx == 0:
            ...
            est_derev = out['x_derev']
            est_rir = out['rir']
            loss = self.loss_func["val"](est_sources, est_derev, est_rir, targets, reverb_targets, rir_target)
            self.log(
                "val_loss",
                loss,
                on_epoch=True,
        ...
        if (self.trainer.current_epoch) % 10 == 0 and dataloader_idx == 1:
            ...
            est_derev = out['x_derev']
            est_rir = out['rir']
            tloss = self.loss_func["val"](est_sources, est_derev, est_rir, targets, reverb_targets, rir_target)
            self.log(
                "test_loss",
                tloss,
                on_epoch=True,
