...
class PITLossWrapper_test(nn.Module):
    def __init__(
    ...
    def forward(self, ests, targets, return_ests=False, reduce_kwargs=None, **kwargs):
        ...
        self.pit_from == "no_pit" # 固定测试
        # ---- 1) 不做 PIT：固定按 target 顺序对齐 ----
        if self.pit_from == "no_pit":
            ...
            # Scalar loss.
                ...
                min_loss = pw_loss.expand(B)                    # [B]

        # ---- 2) 原 PIT 逻辑保持不变 ----
        elif self.pit_from == "pw_mtx":
            pw_loss = self.loss_func(ests, targets, **kwargs)
            assert pw_loss.ndim == 3, "Loss must return [B, N, N] when pit_from='pw_mtx'."
            assert pw_loss.shape[0] == B, "PIT loss needs same batch dim as input"
            ...
            assert pw_loss.ndim == 3, "Loss must return [B, N, N] when pit_from='pw_pt'."
            assert pw_loss.shape[0] == B, "PIT loss needs same batch dim as input"
            min_loss, batch_indices = self.find_best_perm(
                pw_loss, perm_reduce=self.perm_reduce, **reduce_kwargs
            )
