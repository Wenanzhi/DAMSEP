...
def load_wav(filename: str, sr_target: int = 16000) -> torch.Tensor:
    ...
    assert sr_raw == sr_target, (
        f"sample rate not match: got {sr_raw}, expected {sr_target}"
    )
    assert n_ch in [1, 2], f"only 1-ch or 2-ch supported, but got {n_ch}-ch"

    wav, _ = torchaudio.load(wav_path, channels_first=True)  # [C, T]

    if n_ch == 1:
        wav = wav.squeeze(0)   # [T]
    # n_ch == 2 时保持 [2, T]，不要 squeeze
