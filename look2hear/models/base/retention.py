...
class RetNetRelPos(nn.Module):
    ...
    def __init__(self,
                 ...
                 decay: Union[int, bool, List[int], List[float]] = None):
        super().__init__()
        ...
        if decay == False:
            ...
            if decay is None or decay == True:
                decay = 5
            self.decays = (1 - 2**(
                -decay - torch.arange(num_heads, dtype=torch.float))).tolist()
    ...
    def forward(self,
        ...
        if activate_recurrent:
            ...
            block_index = torch.arange(self.recurrent_chunk_size).to(
                self.decay)
            mask = torch.tril(
                torch.ones(self.recurrent_chunk_size,
                           self.recurrent_chunk_size).to(self.decay))
            mask = torch.masked_fill(
                block_index[:, None] - block_index[None, :], ~mask.bool(),
                float("inf"))
            ...
            retention_rel_pos = ((sin, cos),
                                 (inner_mask, cross_decay, query_inner_decay,
                                  value_inner_decay))
        else:
            index = torch.arange(slen).to(self.decay)
            sin = torch.sin(index[:, None] * self.angle[None, :])
            ...
            mask = torch.masked_fill(index[:, None] - index[None, :],
                                     ~mask.bool(), float("inf"))
            mask = torch.exp(mask * self.decay[:, None, None])
            mask = torch.nan_to_num(mask)
...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(self,
        ...
        self.k_proj = nn.Linear(embed_dim, embed_dim,
                                bias=False) if share_qk == False else None
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.k_proj.weight, gain=2**
                                -2.5) if self.share_qk == False else None
    ...
    def parallel_forward(self, qr, kr, v, mask):
        ...
        qk_mat = qk_mat / qk_mat.detach().abs().sum(
            dim=-1, keepdim=True).clamp(min=1, max=5e4)
        output = torch.matmul(qk_mat, vr)
        output = output.transpose(1, 2)
        return output

    def recurrent_forward(self, qr: Tensor, kr: Tensor, v: Tensor,
                          decay: Tensor, incremental_state: Dict[str, Any]):
        bsz = v.size(0)
        ...
        if "prev_key_value" in incremental_state:
            ...
            kv = prev_kv * (prev_scale.sqrt() * decay / scale.sqrt()).view(
                self.num_heads, 1, 1) + kv / scale.sqrt().view(
                    self.num_heads, 1, 1)
    ...
    def chunk_recurrent_forward(self, qr: Tensor, kr: Tensor, v: Tensor,
                                inner_mask):
        ...
        if tgt_len % chunk_len != 0:
            qr = torch.nn.functional.pad(qr,
                                         pad=(0, 0, 0, chunk_len -
                                              (tgt_len % chunk_len)))
            kr = torch.nn.functional.pad(kr,
                                         pad=(0, 0, 0, chunk_len -
                                              (tgt_len % chunk_len)))
            v = torch.nn.functional.pad(v,
                                        pad=(0, 0, 0, chunk_len -
                                             (tgt_len % chunk_len)))
            bsz, tgt_len, embed_dim = v.size()
        ...
        qr = qr.view(bsz, self.num_heads, num_chunks, chunk_len,
                     self.key_dim).transpose(1, 2)
        kr = kr.view(bsz, self.num_heads, num_chunks, chunk_len,
                     self.key_dim).transpose(1, 2)
        ...
        qk_mat = qk_mat * mask
        inner_scale = qk_mat.detach().abs().sum(dim=-1,
                                                keepdim=True).clamp(min=1)
        qk_mat = qk_mat / inner_scale
        inner_output = torch.matmul(
            qk_mat, v
        )  # bsz * num_chunks * num_heads * num_value_heads * chunk_len * head_dim
        ...
        kv_recurrent = []
        cross_scale = []
        kv_state = torch.zeros(bsz, self.num_heads, self.key_dim,
                               self.head_dim).to(v)
        ...
        if tgt_len0 != tgt_len:
            output = output.reshape(bsz, num_chunks * chunk_len,
    ...
    def forward(self,
        ...
        if self.share_qk == False:
            k *= self.scaling
            k = k.view(bsz, tgt_len, self.num_heads,
                       self.key_dim).transpose(1, 2)
        else:
            k = q
        ...
        if incremental_state is not None:
            output = self.recurrent_forward(qr, kr, v, inner_mask,
                                            incremental_state)
        elif chunkwise_recurrent:
    ...
    def extra_repr(self) -> str:
        return f"num_heads={self.num_heads}, share_qk={self.share_qk}" + (
            f", look_ahead={self.look_ahead}" if self.look_ahead > 0 else "")


if __name__ == '__main__':
    embed_dim, num_heads, slen, look_ahead = 96, 4, 251, 2
    ret_pos = RetNetRelPos(embed_dim=embed_dim,
                           num_heads=num_heads,
    ...
    msret = MultiScaleRetention(embed_dim=embed_dim,
                                num_heads=num_heads,
                                value_factor=2,
    ...
    print(y.shape)
    # chunkwise forward
    yc = msret.forward(x=x,
                       rel_pos=ret_pos.forward(slen=slen + look_ahead,
    ...
    if look_ahead == 0:
        ...
        for t in range(x.shape[1]):
            yr = msret.forward(x=x[:, [t], :],
                               rel_pos=ret_pos.forward(
                                   slen=t, activate_recurrent=True),
    ...
    mhsa = MultiheadAttention(embed_dim=embed_dim,
                              num_heads=num_heads,
                              batch_first=True)
