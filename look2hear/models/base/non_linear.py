...
class PReLU(nn.PReLU):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.dim == 1:
            ...
            return super().forward(input.transpose(self.dim,
                                                   1)).transpose(self.dim, 1)


def new_non_linear(non_linear_type: str, dim_hidden: int,
                   seq_last: bool) -> nn.Module:
    if non_linear_type.lower() == 'prelu':
        return PReLU(num_parameters=dim_hidden,
                     dim=1 if seq_last == True else -1)
